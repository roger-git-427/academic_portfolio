...
def load_data(data_dir: Optional[Path] = None) -> Dict[str, pd.DataFrame]:
    """
    Carga todos los DataFrames y devuelve un diccionario con DataFrames ya listos para usar.
    Si USE_DIRECT_DB=true en .env, usa conexión directa a la BD, de lo contrario usa la API.
    """
    if use_direct_db:
        # Use direct database connection
        from .db import load_data_from_db
        print("[INFO] Using direct database connection to load data")
        return load_data_from_db()
    else:
        # Use API as before
        print("[INFO] Using API to load data")
