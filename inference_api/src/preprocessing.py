...
def impute_and_scale(df: pd.DataFrame, features: list[str], scaler_path: str) -> Tuple[np.ndarray, list[str]]:
    """
    1) Impute missing lag_/rolling_ columns using Gaussian noise from last 7 observed rows
    2) Interpolate any remaining NaNs
    3) Apply an existing StandardScaler loaded from disk
    """
    ...
    for col in [c for c in features if c.startswith(("lag_", "rolling_"))]:
        ...
        for i in range(len(s)):
            if pd.isna(s.iat[i]):
                ...
                s.iat[i] = np.random.normal(window.mean(), window.std()) if len(window) > 0 else 0.0
        df[col] = s

    print(f"[DEBUG][impute_and_scale] After imputation, df[features] shape: {df[features].shape}")

    # Interpolate any remaining NaNs
    df = df.interpolate()
    print(f"[DEBUG][impute_and_scale] After interpolation, df[features] shape: {df[features].shape}")

    # Load the scaler
    with open(scaler_path, "rb") as f:
        scaler: StandardScaler = pickle.load(f)

    # Apply transformation using existing scaler
...
def inverse_transform_target(scaler_path: str, y_scaled: np.ndarray) -> np.ndarray:
    """
    Load a StandardScaler from disk and apply inverse_transform
    on a 1D array of scaled target values.
    """
    # Load the saved scaler
    with open(scaler_path, "rb") as f:
        scaler = pickle.load(f)

    # Reconstruct dummy array to match scaler input shape
